...
ner.prototype.fromFile = function(filename, callback) {
	var scope = this;
	var exec = require('child_process').exec;
	console.log('java -mx1500m -cp '+path.normalize(this.options.install_path+'/'+this.options.jar)+' edu.stanford.nlp.ie.crf.CRFClassifier -loadClassifier '+path.normalize(this.options.install_path+'/classifiers/'+this.options.classifier)+' -textFile '+filename);
